let _allowQueryString, _allowExternalLinks, _useWhitelist, _lastTouchTimestamp, _mouseoverTimer, _chromiumMajorVersionInUserAgent = null,
    _delayOnHover = 65,
...
function init() {
    ...
    if (t > -1 && (_chromiumMajorVersionInUserAgent = parseInt(navigator.userAgent.substring(t + 7))), e && _chromiumMajorVersionInUserAgent && _chromiumMajorVersionInUserAgent < 110) return;
    ...
    if ("instantIntensity" in document.body.dataset) {
        ...
        if (e.startsWith("mousedown")) i = !0, "mousedown-only" === e && (r = !0);
...
function isPreloadable(e) {
    if (e && e.href && (!_useWhitelist || "instant" in e.dataset)) {
        if (e.origin !== location.origin) {
            if (!(_allowExternalLinks || "instant" in e.dataset || e.host.endsWith("solo.to")) || !_chromiumMajorVersionInUserAgent) return
        }
        if (["http:", "https:"].includes(e.protocol) && ("http:" !== e.protocol || "https:" !== location.protocol) && (_allowQueryString || !e.search || "instant" in e.dataset) && (!e.hash && !e.href.endsWith("#") || e.pathname + e.search !== location.pathname + location.search) && !("noInstant" in e.dataset)) return !0
    }
}
